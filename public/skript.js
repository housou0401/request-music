...
function renderCarousel(list) {
  ...
  const approxCard = track.querySelector(".result-card");
  const cardRect = approxCard ? approxCard.getBoundingClientRect() : { width: wrapRect.width * 0.18 };
  const padW = Math.max(0, (wrapRect.width - cardRect.width)/2);
  const spL = document.createElement("div"); spL.style.flex = `0 0 ${padW}px`;
  const spR = document.createElement("div"); spR.style.flex = `0 0 ${padW}px`;
  track.prepend(spL); track.appendChild(spR);
currentList.forEach((s, i)=> {
  ...
  function update3D() {
    ...
    cards.forEach((c, idx)=>{
      ...
      const scale = 0.84 + Math.max(0, 0.30 * (1 - Math.min(1, dist*2))); // center~1.14 / side~0.84 // center~1.08, side~0.80
      const ry = -18 * dx; // 左右に少し傾ける
      c.style.setProperty("--scale", scale.toFixed(3));
      c.style.setProperty("--ry", ry.toFixed(3) + "deg");
      if (dist < nearest.d) nearest = {i: idx, d: dist};
    });
    // 選択のハイライト
    $$(".result-card").forEach(c => { c.classList.remove("selected"); c.setAttribute("aria-selected","false"); });
    if (nearest.i >= 0) {
      $$(".result-card")[nearest.i].classList.add("selected");
    }
  }
  let scrollSnapTimer=null; wrap.addEventListener("scroll", ()=>{ update3D(); clearTimeout(scrollSnapTimer); scrollSnapTimer=setTimeout(()=>{ const cards = $$(".result-card"); if (!cards.length) return; const rect = wrap.getBoundingClientRect(); const center = rect.left + rect.width/2; let nearest={i:-1,d:1e9}; cards.forEach((c,idx)=>{ const r=c.getBoundingClientRect(); const mid=r.left+r.width/2; const d=Math.abs(mid-center); if (d<nearest.d) nearest={i:idx,d}; }); if (nearest.i>=0) selectCarouselIndex(nearest.i,false); }, 90); }, {passive:true});
...
// Transport controls (prev/next + big play)
function selectPrev(){ if (currentIndex > 0) selectCarouselIndex(currentIndex-1, true); }
function selectNext(){ if (currentIndex+1 < currentList.length) selectCarouselIndex(currentIndex+1, true); }

function setupTransportButtons(){
  const prev = document.getElementById("prevBtn");
  const next = document.getElementById("nextBtn");
  const big  = document.getElementById("playPauseBtnBig");
  const play = document.getElementById("playPauseBtn");
  const el   = AudioManager.element();

  if (prev) prev.addEventListener("click", ()=> selectPrev());
  if (next) next.addEventListener("click", ()=> selectNext());
  if (big)  big.addEventListener("click", async ()=>{
    if (el.paused) { try { await AudioManager.play(); big.textContent = "⏸"; if (play) play.textContent = "⏸"; } catch{} }
    else { AudioManager.pause(false); big.textContent = "⏯"; if (play) play.textContent = "▶"; }
  });

  // keep big/play in sync
  el.addEventListener("play", ()=>{ if (big) big.textContent = "⏸"; if (play) play.textContent="⏸"; });
  el.addEventListener("pause", ()=>{ if (big) big.textContent = "⏯"; if (play) play.textContent="▶"; });
}

window.addEventListener("DOMContentLoaded", setupTransportButtons);
