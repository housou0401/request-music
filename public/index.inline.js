...
async function applySettingsToUI() {
    try {
        ...
        if (!data.recruiting) {
            if (titleEl) {
                titleEl.textContent = "現在募集を終了しています";
                titleEl.style.color = "#d00000";
            }
            if (modeToggle)
                modeToggle.classList.add("ux-hidden");
            if (form)
                Array.from(form.querySelectorAll("input,button,select,textarea")).forEach(el => el.disabled = true);
            if (stopHost) {
                stopHost.innerHTML = `
            ...
            if (form)
                Array.from(form.querySelectorAll("input,button,select,textarea")).forEach(el => el.disabled = false);
            stopHost && (stopHost.innerHTML = "");
        }
        if (data.frontendTitle)
            document.getElementById("frontendTitle").textContent = data.frontendTitle;
    }
    catch (e) {
...
function _ovShow(id) { const el = document.getElementById(id); if (el) {
    el.classList.add('show');
    el.setAttribute('aria-hidden', 'false');
} }
function _ovHide(id) { const el = document.getElementById(id); if (el) {
    el.classList.remove('show');
    el.setAttribute('aria-hidden', 'true');
} }
function _setText(id, txt) { const el = document.getElementById(id); if (el)
...
function _disableMainForm(disabled) {
    ...
    form.querySelectorAll('input,button,select,textarea').forEach(el => {
        if (el.id === 'adminBtn')
            return;
        if (el.id === 'reqTermsOpenBtn' || el.id === 'reqTermsCancelBtn' || el.id === 'reqTermsAgreeBtn')
            return;
        if (el.id === 'warningOkBtn' || el.id === 'warningCloseX' || el.id === 'banOkBtn' || el.id === 'banCloseX')
            return;
        try {
...
function applyPenaltyUI(me) {
    try {
        ...
        if (me.adminSession && me.impersonating) {
...
async function ackWarningAndClose() {
    try {
        await fetch('/penalty/warning-ack', { method: 'POST' });
    }
    catch { }
    _ovHide('warningOverlay');
}
document.addEventListener('click', (e) => {
    ...
    if (!t)
        return;
    if (t.id === 'warningOkBtn') {
        ackWarningAndClose();
    }
    if (t.id === 'warningCloseX') {
        ackWarningAndClose();
    }
    if (t.id === 'banOkBtn' || t.id === 'banCloseX') {
        _ovHide('banOverlay');
    }
});
...
async function ensureReqTermsLoaded(force = false) {
    if (force || !__reqTermsCache)
        __reqTermsCache = await fetchRequestTerms();
...
async function openReqTermsModal(type, payload) {
    ...
    if (box)
        box.textContent = rt.termsText || '（利用規約が設定されていません）';
    showReqTermsOverlay();
}
async function acceptReqTermsAndContinue() {
    ...
    if (pending.type === 'submit') {
        try {
            ...
            try {
                sessionStorage.removeItem('reqTermsDismissedVersion');
            }
            catch { }
...
async function maybeShowUpdatedRequestTerms(me) {
    try {
        ...
        if (me.adminSession === true || role === 'admin' || role === 'site_admin')
            return;
...
async function refreshAdminTryInfo() {
    try {
        ...
        if (el)
            el.textContent = `管理者パスワード残り: ${left} 回`;
        return left;
    }
    catch {
        const el = document.getElementById('adminTryInfo');
        if (el)
            el.textContent = `管理者パスワード残り: 3 回`;
        return 3;
    }
}
...
function wireLegacyPlayerControls() {
    ...
    if (!host)
        return;
    const playBtn = host.querySelector('button.play, button.play-btn, button#playButton, button[title*="再生"], button[aria-label*="再生"], button') ||
        null;
    let vol = null;
    ...
    for (const r of ranges) {
        ...
        if (max === '1' || step === '0.01' || r.id?.toLowerCase().includes('volume')) {
            vol = r;
            break;
        }
    }
    if (!vol && ranges.length === 1)
        vol = ranges[0];
    if (playBtn) {
    ...
    if (vol) {
        ...
        vol._bound = (e) => {
            ...
            if (!Number.isNaN(v)) {
                lastVolume = Math.min(1, Math.max(0, v));
                audioEl.volume = lastVolume;
            }
        };
        vol.addEventListener('input', vol._bound);
        try {
            if (typeof vol.value !== 'undefined')
...
async function startRefillCountdown() {
    try {
        ...
        function fmtLeft(ms) {
            ...
            if (days > 0)
                return `次回配布まで: ${days}日 ${String(hrs).padStart(2, "0")}:${String(mins).padStart(2, "0")}:${String(secs).padStart(2, "0")}`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (reqForm) {
        reqForm.addEventListener('submit', async (ev) => {
            if (window.__reqTermsBypassOnce) {
                window.__reqTermsBypassOnce = false;
                return;
            }
            const me = await fetchMe();
            if (!me || !me.loggedIn)
