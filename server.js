...
const COOKIE_OPTS = { httpOnly: true, sameSite: "Lax", path: "/", maxAge: 1000 * 60 * 60 * 24 * 365 };
...
app.post("/mypage/update", async (req, res) => {
  ...
  if (usersDb.data.users.some(x => x.id !== u.id && String(x?.username ?? "").trim() === name)) {
    return res.send(toastPage("⚠そのユーザー名はすでに使用されています。管理運営上、同じユーザー名を使用することはできません。", "/mypage"));
  }
  u.username = name;
  await usersDb.write();
