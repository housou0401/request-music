...
app.use(async (req, _res, next) => {
  ...
  // ---- admin 判定（2系統）----
  const adminAuth = (req.cookies?.adminAuth === "1");
  const adminUser = (baseUser && isAdmin(baseUser));

  // adminSession: /admin など「管理画面」に入れる権限（＝ログイン中(ベース)が管理者のときのみ）
  const adminSession = !!adminUser;
  // adminDevice: 「管理者が一度でもログインした端末」(adminAuth) または管理者ログイン中
  const adminDevice = !!adminUser || adminAuth;
...
// ---- 管理端末保護（ログアウト/切替など端末操作） ----
function requireAdminDevice(req, res, next) {
  if (req.adminDevice) return next();
  return res
    .status(403)
    .send(`<!doctype html><meta charset="utf-8"><title>403</title><p>この端末では利用できません。</p><p><a href="/">トップへ</a></p>`);
}
...
// ==========================
// 管理端末: ログアウト / ユーザー切替 / 端末初期化
// ==========================

// 管理端末：ログアウト（deviceId だけ消す / adminAuth は残す）
app.post("/admin/device/logout", requireAdminDevice, (req, res) => {
  res.clearCookie("deviceId");
  res.clearCookie("impersonateId");
  res.clearCookie(TOK_COOKIE);
  return res.json({ ok: true });
});
app.get("/admin/device/logout", requireAdminDevice, (req, res) => {
  res.clearCookie("deviceId");
  res.clearCookie("impersonateId");
  res.clearCookie(TOK_COOKIE);
  return res.redirect("/?switch=1");
});

// 管理者のみ：端末初期化（端末のアカウント情報 + users 登録も削除）
app.post("/admin/device/wipe", requireAdmin, async (req, res) => {
  try {
    const baseId = req.cookies?.deviceId;
    if (!baseId) return res.status(400).json({ ok: false, error: "missing deviceId" });

    await usersDb.read();
    usersDb.data.users = usersDb.data.users.filter(u => u.id !== baseId);
    await usersDb.write();
    ...
    res.clearCookie("areg");
    res.clearCookie("alog");
    return res.json({ ok: true });
  } catch (e) {
    return res.status(500).json({ ok: false, error: e.message });
  }
});
app.get("/admin/device/wipe", requireAdmin, async (req, res) => {
  try {
    const baseId = req.cookies?.deviceId;
    if (baseId) {
      await usersDb.read();
...
app.get("/mypage", async (req, res) => {
  ...
  <body>
    <div class="wrap">
      ...
      <div class="card">
        <h3>管理端末ツール</h3>
        <p class="muted">この端末が管理端末として記録されている場合のみ表示されます。<br>ログアウト/ユーザー切替は、現在のユーザーのロールに関係なく利用できます。</p>
        <div class="row" style="flex-wrap:wrap;">
          <button class="btn" type="button" id="btnDeviceLogout" style="border-color:#fecaca;background:#fff5f5;color:#b91c1c;">ログアウト（この端末）</button>
          <button class="btn" type="button" id="btnDeviceSwitch">ユーザー切替</button>
          ${req.adminSession ? `<button class="btn" type="button" id="btnDeviceWipe" style="border-color:#ef4444;background:#fee2e2;color:#b91c1c;">端末初期化（ユーザー削除）</button>` : ``}
        </div>
        <div class="muted" style="font-size:12px;margin-top:8px;">※ユーザー切替は一度ログアウトして、ログイン画面で別ユーザーを選びます。</div>
      </div>
      ` : ``}
      <p><a href="/">↩ トップへ戻る</a></p>
    </div>
    ...
    (function(){
      ...
      // ---- 管理端末ツール（ログアウト/切替/端末初期化） ----
      const adminDevice = ${req.adminDevice ? "true" : "false"};
      const canWipe = ${req.adminSession ? "true" : "false"};
      if (adminDevice) {
        const doDeviceLogout = async () => {
          try{
            const r = await fetch("/admin/device/logout", { method:"POST" });
            const d = await r.json().catch(()=>({}));
            if (!r.ok || !d.ok) return alert("ログアウトに失敗しました");
        ...
        if (bL) bL.addEventListener("click", ()=>{
          if (!confirm("この端末からログアウトしますか？（管理端末フラグは残ります）")) return;
          doDeviceLogout();
        });
        if (bS) bS.addEventListener("click", ()=>{
          if (!confirm("ユーザーを切り替えるため、一度ログアウトします。よろしいですか？")) return;
          doDeviceLogout();
        });

        const bW = document.getElementById("btnDeviceWipe");
        if (bW && canWipe) bW.addEventListener("click", async ()=>{
          if (!confirm("⚠この端末のアカウント情報を完全削除します（usersからも削除）。元に戻せません。よろしいですか？")) return;
          try{ try{ localStorage.removeItem("rm_known_accounts_v1"); }catch{} }catch{}
          try{
            const r = await fetch("/admin/device/wipe", { method:"POST" });
            const d = await r.json().catch(()=>({}));
            if (!r.ok || !d.ok) return alert("削除に失敗しました");
            location.href = "/";
          }catch(e){
            alert("通信エラー: " + e.message);
          }
