...
const fetchResultsForQuery = async (query, lang, entity = "song", attribute = "") => {
  let url = `https://itunes.apple.com/search?term=${encodeURIComponent(query)}&country=JP&media=music&entity=${entity}&limit=50&explicit=no&lang=${lang}` + (attribute ? `&attribute=${attribute}` : "");
...
function adminTogglePlay(id) {
  ...
  if (adminIsPlayingMap[id]) { fadeOutAudio(id, 200); }
  else { adminAudioMap[id].muted = false; adminIsMutedMap[id] = false; adminAudioMap[id].play(); adminIsPlayingMap[id] = true; fadeInAudio(id, 0.5, 750); }
  updateAdminPlayIcon(id);
  updateAdminMuteIcon(id);
}
function fadeInAudio(id, finalVolume, duration) {
  const steps = 30; const stepTime = duration / steps; let currentStep = 0; const stepVol = finalVolume / steps;
...
function fadeOutAudio(id, duration) {
  ...
  const steps = 10; const stepTime = duration / steps; let currentStep = 0; const initialVolume = adminAudioMap[id].volume; const stepVol = initialVolume / steps;
...
function adminChangeVolume(id, val) {
  ...
  if (volume > 0 && adminIsMutedMap[id]) { adminAudioMap[id].muted = false; adminIsMutedMap[id] = false; }
  updateAdminMuteIcon(id);
}
function createPaginationLinks(currentPage, totalPages) {
  let html = `<div style="text-align:left; margin-bottom:10px;">`;
